...
class MaskHead(nn.Module):
    ...
    def forward(self, detections, features):
        ...
        for detection in detections:
            ...
            up1 = self.up1(f_8[b, :, int(z_start / 8):int(z_end / 8), int(y_start / 8):int(y_end / 8), int(x_start / 8):int(x_end / 8)].unsqueeze(0))
            up1 = self.back1(torch.cat((up1, f_4[b, :, int(z_start / 4):int(z_end / 4), int(y_start / 4):int(y_end / 4), int(x_start / 4):int(x_end / 4)].unsqueeze(0)), 1))
            up2 = self.up2(up1)
            up2 = self.back2(torch.cat((up2, f_2[b, :, int(z_start / 2):int(z_end / 2), int(y_start / 2):int(y_end / 2), int(x_start / 2):int(x_end / 2)].unsqueeze(0)), 1))
            up3 = self.up3(up2)
