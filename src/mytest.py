...
from utils.visualize import draw_gt, draw_pred, generate_image_anim, generate_image_niis
from utils.util import dice_score_seperate, get_contours_from_masks, merge_contours, hausdorff_distance
from utils.util import onehot2multi_mask, normalize, pad2factor, load_nrrd_image, crop_boxes2mask
...
parser.add_argument("--nrrd-path", type=str, default=None,
                    help="path to nrrd files of patient")
...
def main():
    ...
    if args.mode == 'test':
        if args.weight == None:
            ...
            return -1
        if args.nrrd_path == None:
            logging.error('nrrd file path must be specified if using test mode')
            return -1
        if args.out_dir == None:
            logging.error('output directory must be specified if using test mode')
            return -1
        test_single(args.weight, args.nrrd_path, args.out_dir,
...
def test_single(weight_path, file_path, save_dir, do_postprocess=False, anchor_params=None):
    ...
    logging.info('Starting processing, nrrd file path %s, output dir %s' % (file_path, save_dir))
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    # Load image data
    standard_spacing = np.array([2.5, 0.976562, 0.976562])
    raw_img, origin, spacing = load_nrrd_image(file_path)
    ...
    nii_save_dir = os.path.join(save_dir)
    if not os.path.exists(nii_save_dir):
        os.makedirs(nii_save_dir)
    logging.info('Saving predicted niis to %s' % (nii_save_dir))
    generate_image_niis(raw_img, raw_pred_mask, nii_save_dir)
