...
def detections2mask(detections, masks, img_reso, num_class=30):
    """
...
def crop_boxes2mask(crop_boxes, masks, img_reso, num_class=30):
    """
    Apply results of mask-rcnn (detections and masks) to mask result.
...
def load_nrrd_image(filename):
    ...
    # Since NRRD files usually contain a single 3D image, we don't need a series of filenames
    # Get the image origin and spacing
    numpyOrigin = np.array(list(reversed(itkimage.GetOrigin())))
    numpySpacing = np.array(list(reversed(itkimage.GetSpacing())))
