        ...
        with torch.no_grad():
            ...
            for patient_order in range(patient_number):
                ...
                if save :
                    ...
                    # print(totalpredic.shape)
                    
                    # target_shape = (totalpredic.shape[0], 1024, 1024)

                    # upsampled_totalpredic = np.zeros(target_shape)

                    # for z in range(totalpredic.shape[0]):
                    #     slice_to_upsample = totalpredic[z, :, :]
                    #     upsampled_slice = cv2.resize(slice_to_upsample, (target_shape[2], target_shape[1]), interpolation=cv2.INTER_NEAREST)
                    #     upsampled_totalpredic[z, :, :] = upsampled_slice
                    # save_array_as_nifty_volume(upsampled_totalpredic, patient_path + '/Seg.nii.gz')
